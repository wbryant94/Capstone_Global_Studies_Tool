...
export const getProfessorDepartment = (req, res) => {
  ...
  const q =
    req.params.id && req.params.id !== "All Departments"
 ...
 */
  ...
  const [result, fieldData] = await db
    .promise()
    .query(checkQuery, [req.body.inputs.email]);

    console.log("result of matching emails: ",result)
  if (result.length) {
    console.log("result:", result[0].professor_id);
    return res.status(409).json({
      professor: result[0],
      message: "Professor Already Exists With Email: " + req.body.inputs.email,
    });
  }

  console.log("outside of building insertQuery")
  const insertQuery =
    "INSERT INTO professors(`fname`,`lname`, `department`, `description`, `email`, `image`) VALUES (?)";
  const values = [
    req.body.inputs.fname,
    req.body.inputs.lname,
    req.body.inputs.department,
    req.body.inputs.description,
    req.body.inputs.email,
    req.body.img,
  ];

console.log("outside of insert fname:",req.body.inputs.fname)
...
export const addResource = (req, res) => {
  console.log("in add resource:", req.body);
  const countryIds = Object.values(req.body.countries).map(
    (item) => item.value
  );
...
export const updateProfessor = async (req, res) => {
  ...
  if (result.length && result[0].professor_id != req.body.inputs.professor_id) {
    console.log("result:", result[0].professor_id);
    return res.status(409).json({
      message: "Professor Already Exists With Email: " + req.body.inputs.email,
    });
...
console.log("made it to update")
  const updateQuery = `UPDATE professors SET fname='${req.body.inputs.fname}', lname='${req.body.inputs.lname}',department='${req.body.inputs.department}',description='${req.body.inputs.description}',email='${req.body.inputs.email}',image='${req.body.img}' WHERE professor_id=${req.body.inputs.professor_id}`;
...
/*   console.log("deleteResource:", deleteResourcesQuery); */
  ...
  if (toDelete.length > 0) {
    db.query(deleteResourcesQuery, (err, data) => {
      ...
      console.log("Successfully Updated Professor's Connects")
      return res.status(200).json(data);
    });
  }
};
...
export const getConnectedCountries = (req, res) => {
  const q = `SELECT c.name, c.country_id, p.* FROM countries as c 
  JOIN connections 
